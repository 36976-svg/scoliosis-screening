...
def measure_zone_brightness(image_bgr, annotated, y_top, y_bottom, x_center, w,
    ...
    เช่น Back Asymmetry หรือสะบักนูน) แล้ววาดกรอบ+ป้ายกำกับลงบนภาพ
...
def analyze_standing(image_bgr):
    ...
    # Back Asymmetry (ท่ายืน): แบ่งแผ่นหลังเป็นโซนย่อยแยกกัน แทนที่จะเฉลี่ยรวมเป็นก้อนเดียว
    # เพื่อให้รู้ว่านูนตรงระดับไหนจริงๆ (โซนสะบักมีตัวจับเฉพาะแยกต่างหากอยู่แล้วด้านล่าง
    # จึงตัดออกจากตรงนี้ กันไม่ให้นับสัญญาณเดียวกันซ้ำสองที่)
    # หมายเหตุ: นี่ไม่ใช่ Rib Hump ตามนิยามทางการแพทย์ (ซึ่งต้องวัดจากท่าก้มตัว/Adam's Test เท่านั้น)
    # เป็นแค่ค่าความไม่สมมาตรทั่วไปของผิวหลังตอนยืนตรง ใช้ชื่อนี้เพื่อไม่ให้เข้าใจผิด
    back_span = mid_hip_y - mid_shoulder_y
    midback_top    = mid_shoulder_y + int(SCAPULA_ZONE_FRAC * back_span)
    midback_bottom = mid_shoulder_y + int(0.75 * back_span)
    lowerback_top    = midback_bottom
    lowerback_bottom = mid_hip_y

    midback_diff, midback_side = measure_zone_brightness(
        image_bgr, annotated, midback_top, midback_bottom, mid_shoulder_x, w,
        label="Mid-back", label_y_offset=14, box_color_hint=(255, 200, 0))
    lowerback_diff, lowerback_side = measure_zone_brightness(
        image_bgr, annotated, lowerback_top, lowerback_bottom, mid_shoulder_x, w,
        label="Lower-back", label_y_offset=14, box_color_hint=(0, 140, 255))

    if midback_diff >= lowerback_diff:
        rib_hump_diff, rib_hump_side, rib_hump_zone = midback_diff, midback_side, "กลางหลัง"
    else:
        rib_hump_diff, rib_hump_side, rib_hump_zone = lowerback_diff, lowerback_side, "หลังส่วนล่าง/เอว"
    ...
    return {
        ...
        "rib_hump_zone":        rib_hump_zone,
        "midback_diff":         midback_diff,
        "midback_side":         midback_side,
        "lowerback_diff":       lowerback_diff,
        "lowerback_side":       lowerback_side,
...
RIB_HUMP_LOW, RIB_HUMP_HIGH = 3.0, 8.0        # Back Asymmetry (ท่ายืน): ความต่างความสว่างซ้าย-ขวา (สเกล 0-255)
SCAPULA_DIFF_LOW, SCAPULA_DIFF_HIGH = 3.0, 8.0  # ความนูนต่างกัน (สเกลเดียวกับ Back Asymmetry)
    ...
    if uploaded_files:
        ...
        if rows:
            ...
            for key, label in [
                ...
                ("rib_hump_diff",    "Back Asymmetry (ท่ายืน)"),
        ...
        if result is None:
            ...
            st.image(annotated_rgb,
                     caption="จุดแดง = ไหล่ (และสะโพกถ้าเห็นในภาพ) | จุดม่วง = เอว (หาจากรูปทรงลำตัว) | เส้นเหลือง = แนวกระดูกสันหลัง (คอ→ไหล่→อก/เอว→สะโพกหรือขอบภาพ) | "
                             ...
                             "กรอบฟ้าอ่อน = Mid-back ด้านที่นูนกว่า | กรอบส้ม = Lower-back/เอว ด้านที่นูนกว่า | กรอบเขียว = Scapula (สะบัก) ด้านที่นูนกว่า — ทั้งหมดนี้ไม่ใช่ Rib Hump ทางการแพทย์ ต้องวัดจากท่าก้มตัวเท่านั้น",
            ...
            with col4:
                st.metric("Back Asymmetry (ท่ายืน)", f"{result['rib_hump_diff']:.1f}")
                st.markdown(f"ความเสี่ยง: :{rib_hump_display_color}[{rib_hump_risk}]")
                st.caption("⚠️ ไม่ใช่ Rib Hump ทางการแพทย์ (ต้องวัดจากท่าก้มตัว/Adam's Test) "
                           "นี่คือความไม่สมมาตรผิวหลังตอนยืนตรงเท่านั้น")
            with col5:
                st.write(f"**ด้านที่นูนกว่า:** {result['rib_hump_side']}")
                st.write(f"**โซนที่นูนชัดสุด:** {result['rib_hump_zone']}")
                st.caption(f"กลางหลัง: {result['midback_diff']:.1f} ({result['midback_side']}) | "
                           f"หลังส่วนล่าง/เอว: {result['lowerback_diff']:.1f} ({result['lowerback_side']})")
            with col6:
