...
def find_waist_points(image_bgr, y_top, y_bottom,
    ...
    edge_px = max(5, int(w * edge_sample_frac))
    left_xs, right_xs = {}, {}

    for y in range(y_start, y_end):
        row = image_bgr[y].astype(np.float32)
        bg_left  = np.median(row[:edge_px], axis=0)
        bg_right = np.median(row[-edge_px:], axis=0)
        bg_ref = (bg_left + bg_right) / 2.0
        diffs = np.linalg.norm(row - bg_ref, axis=1)
        fg = diffs > bg_dist_threshold

        # หา contiguous run ที่ยาวที่สุดในแถวนี้ (กันจุดรบกวน/แสงสะท้อนเป็นหย่อมๆ)
        best_start, best_len, cur_start, run_len = -1, -1, -1, 0
        ...
        left_xs[y]  = best_start
        right_xs[y] = best_start + best_len - 1
